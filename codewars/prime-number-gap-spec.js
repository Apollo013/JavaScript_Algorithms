/*
    ...
    Examples:
        ...
        gap(6,100,110) --> nil or {0, 0} : between 100 and 110 we have 101, 103, 107, 109 but 101-107is not a 6-gap because there is 103 in between and 103-109 is not a 6-gap because there is 107 in between.
*/

describe('PRIME GAP: =>', function(){
    ...
    var gap = function(g, m, n){
        for(m=m; m <= n-g; m++){
            if(isPrime(m) && isPrime(m+g)){
                for(var j = m + 1; j < m + g; j++){
                    if(isPrime(j)){return null;}
                }
                return [m, m+g];
            }
        }
    ...
    it('Should return [3,5] for gap(2, 2, 50)', function(){
        expect(gap(2, 2, 50)).toEqual([3,5]);
    });

    it('Should return null for gap(2, 5, 5)', function(){
        expect(gap(2, 5, 5)).toBe(null);
    });

    it('Should return [5,7] for gap(2, 5, 7)', function(){
        expect(gap(2, 5, 7)).toEqual([5,7]);
    });

    it('Should return [163, 167] for gap(4, 130, 200)', function(){
        expect(gap(4, 130, 200)).toEqual([163, 167]);
    });

    it('Should return null for gap(6,100,110)', function(){
        expect(gap(6,100,110)).toBe(null);
    });
})